...
(function () {
  ...
  function localChart(wrap,instrument,candles){
    ...
    const rows=(candles||[]).map(r=>({t:r&&r[0],o:+(r&&r[1]),h:+(r&&r[2]),l:+(r&&r[3]),c:+(r&&r[4]),v:+(r&&r[5]||0)})).filter(r=>[r.o,r.h,r.l,r.c].every(Number.isFinite)).slice(-90);
    ...
    function draw(){
      ...
      canvas.width=Math.floor(w*dpr);canvas.height=Math.floor(h*dpr);canvas.style.width=w+'px';canvas.style.height=h+'px';ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);
      const pad={l:58,r:18,t:24,b:34},cw=w-pad.l-pad.r,ch=h-pad.t-pad.b;
      const hi=Math.max(...rows.map(x=>x.h)),lo=Math.min(...rows.map(x=>x.l));const span=Math.max(0.01,hi-lo),y=p=>pad.t+(hi-p)/span*ch,x=i=>pad.l+(i+0.5)*cw/rows.length;
      ctx.font='10px Segoe UI,Arial';ctx.lineWidth=1;ctx.strokeStyle='rgba(80,110,135,.22)';ctx.fillStyle='#8397ab';
      ...
      rows.forEach((r,i)=>{const px=x(i),up=r.c>=r.o;ctx.strokeStyle=up?'#20d18b':'#ff5d6c';ctx.fillStyle=ctx.strokeStyle;ctx.beginPath();ctx.moveTo(px,y(r.h));ctx.lineTo(px,y(r.l));ctx.stroke();const top=y(Math.max(r.o,r.c)),bot=y(Math.min(r.o,r.c));ctx.fillRect(px-body/2,top,body,Math.max(1,bot-top));});
      const last=rows[rows.length-1];ctx.fillStyle='#dcecff';ctx.font='bold 11px Segoe UI,Arial';ctx.fillText((instrument.symbol||instrument.name||'Instrument')+' · '+last.c.toFixed(2),pad.l,15);ctx.fillStyle='#71879b';ctx.font='9px Segoe UI,Arial';ctx.fillText('Angel One OHLC · last '+rows.length+' sessions',pad.l,h-10);const step=Math.max(1,Math.floor(rows.length/5));for(let i=0;i<rows.length;i+=step)ctx.fillText(String(rows[i].t||'').slice(0,10),x(i)-20,h-20);
    }
  ...
  async function loadLocal(wrap,instrument){
    try{
      const r=await fetch(dataEndpoint(instrument),{headers:{'Accept':'application/json'},cache:'no-store'});const d=await r.json();
      if(!r.ok||!d.success)throw new Error(d.message||'Historical data unavailable');const candles=d.data||d.candles||d.history||[];localChart(wrap,instrument,candles);
      const status=document.getElementById('azChartStatus');if(status)status.textContent='Angel One · '+(instrument.symbol||instrument.name||'Instrument')+' · '+candles.length+' candles';const hint=document.getElementById('azChartHint');if(hint)hint.textContent='Instrument-specific Angel One chart. Chart AI reads the same candle data automatically.';return true;
  ...
  function mount(symbol,instrument){
    ...
    window.setTimeout(function(){
      if(wrap.querySelector('.az-canvas'))return;
      const host=document.createElement('div');host.className='tradingview-widget-container';host.style.cssText='height:100%;width:100%;min-height:0;';const widget=document.createElement('div');widget.className='tradingview-widget-container__widget';widget.style.cssText='height:100%;width:100%;min-height:0;';host.appendChild(widget);wrap.innerHTML='';wrap.appendChild(host);
      const script=document.createElement('script');script.src='https://s3.tradingview.com/external-embedding/embed-widget-advanced-chart.js';script.async=true;script.type='text/javascript';script.innerHTML=JSON.stringify({autosize:true,width:'100%',height:'100%',symbol,interval:'D',timezone:'exchange',theme:'dark',style:'1',locale:'en',withdateranges:true,hide_side_toolbar:false,allow_symbol_change:true,show_popup_button:false,save_image:false,calendar:false,support_host:'https://www.tradingview.com',studies:['Volume@tv-basicstudies','RSI@tv-basicstudies','MACD@tv-basicstudies','Moving Average@tv-basicstudies','Moving Average Exponential@tv-basicstudies']});host.appendChild(script);
    },1200);
  }
