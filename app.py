...
def api_scan():
    ...
    _ensure_scan_refresh(force=force)
    with _scan_lock:
        payload = dict(_scan_state["result"] or {})
        refreshing = _scan_state["refreshing"]
        updated_at = _scan_state["updated_at"]
        last_error = _scan_state["last_error"]
    if not payload:
        return jsonify({"success": True, "message": "X10 scan is starting.", "stocks": [], "top_opportunities": [], "indices": [], "refreshing": refreshing, "updated_at": updated_at, "error": last_error})
